...
async def complete_song_analysis_by_url(song_url:str = Body(...),image:UploadFile = File(...),song_division = Depends(get_song_division_service), song_sentiment_analysis = Depends(get_song_sentiment_analysis_service), song_emotion_anaysis = Depends(get_song_emotion_analysis_service), image_to_base64 = Depends(get_image_to_base64_service), image_description = Depends(get_image_description_service), song_complete_analysis = Depends(get_song_complete_analysis_service), genius_scrapping = Depends(get_genius_scrapping_service),client:httpx.AsyncClient = Depends(get_http_client), db_session = Depends(get_session)) -> SongFullAnalysisOutput:
    start = time.time()
